# -*- coding: utf-8 -*-
"""
对每一层、每一个样本，分别计算两种差分向量并做对齐后的余弦相似度：
...
注意：
  ...
  - 对每一层：
      (a) 画一张图：这一层所有样本的 cos 值分布（直方图）
      (b) 计算这一层 cos 的统计信息（数量、均值、方差、分位数等）

输出结构（写死）：
  RESULT_DIR = BASE_HALLU_DIR / "hallu_vs_delta_cosine"
    - figs/layer_00_cos_hist.png, ..., layer_31_cos_hist.png
    - cosine_values_per_layer.npz   # 各层 cos 值数组
    - cosine_stats_per_layer.json   # 各层统计信息
"""
...
# 结果输出目录
RESULT_DIR = os.path.join("/data/ruipeng.zhang/steering/analyse/", "hallu_vs_delta_cosine")
FIG_DIR = os.path.join(RESULT_DIR, "figs")
COS_NPZ_PATH = os.path.join(RESULT_DIR, "cosine_values_per_layer.npz")
STATS_JSON_PATH = os.path.join(RESULT_DIR, "cosine_stats_per_layer.json")
...
def main():
    ...
    for layer_idx in range(NUM_LAYERS):
        ...
        if len(cos_values) == 0:
            print(f"[main] layer_{layer_idx}: no valid cosine values, skip plotting/stat.")
            continue
        ...
        plt.title(f"Layer {layer_idx} cosine distribution (n={len(cos_arr)})")
    ...
    print("\n[done] hallu vs delta cosine analysis finished.")
