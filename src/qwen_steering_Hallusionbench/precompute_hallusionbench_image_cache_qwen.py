...
# -*- coding: utf-8 -*-
"""
离线预处理 HallusionBench 图片并缓存 Qwen2.5-VL 的“视觉侧 inputs”（pixel_values + image_grid_thw）。

【融合逻辑】
- HallusionBench 的数据读取方式参考 LLaVA cache 脚本：
  读取 HallusionBench.json（list），只处理 visual_input == "1" 的样本，
  收集 item["filename"] 去重并保持顺序。
- Qwen2.5-VL 的视觉缓存方式参考你的 MMHal Qwen cache 脚本：
  只加载纯视觉 ImageProcessor（AutoImageProcessor 或 AutoProcessor.image_processor），
  预处理输出 pixel_values + image_grid_thw，允许 pixel_values 为 2D/3D/4D。

【输出】
cache_folder/<filename>.pt
{
...
【典型用法】
python cache_hallusionbench_qwen25vl.py \
  --model-path /data/base_model/.../Qwen2.5-VL-7B-Instruct/... \
  --bench-file /data/.../HallusionBench.json \
  --image-folder /data/.../hallusion_bench \
  --cache-folder /nas_data/.../HallusionBench_pre_cache_qwen25vl \
...
DEFAULT_BENCH_FILE = "/data/ruipeng.zhang/dpo_on/HallusionBench/output/HallusionBench.json"
DEFAULT_IMAGE_FOLDER = "/data/ruipeng.zhang/dpo_on/HallusionBench/hallusion_bench"
DEFAULT_CACHE_FOLDER = "/nas_data/ruipeng.zhang/HallusionBench_pre_cache_qwen25vl"
...
def parse_args():
    ...
    # --- HallusionBench 数据 ---
    p.add_argument("--bench-file", type=str, default=DEFAULT_BENCH_FILE)
    ...
    p.add_argument("--only-images-file", type=str, default="",
                   help="文本文件：每行一个 filename（只处理这些，需与 bench 中一致）")
...
def load_hallusionbench(path: str) -> List[dict]:
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if not isinstance(data, list):
        raise ValueError("HallusionBench file must be a JSON list.")
    if len(data) == 0:
        raise RuntimeError(f"HallusionBench json empty: {path}")
    return data


def is_visual_item(item: Dict[str, Any]) -> bool:
    # 只处理 visual_input == "1"
    return str(item.get("visual_input", "")).strip() == "1"


def parse_filename(item: Dict[str, Any]) -> Optional[str]:
...
def collect_unique_filenames_keep_order(items: List[dict]) -> List[str]:
    seen: Set[str] = set()
    out: List[str] = []
    for item in items:
        if not is_visual_item(item):
            continue
        fn = parse_filename(item)
        if not fn:
            continue
        if fn not in seen:
            seen.add(fn)
...
def select_filenames(filename_list: List[str], args) -> List[str]:
    ...
    if args.only_last_n and args.only_last_n > 0:
        return filename_list[-int(args.only_last_n):]

    if args.limit and args.limit > 0:
        return filename_list[:int(args.limit)]
...
def main():
    ...
    print("=" * 100)
    print("[hallusion-cache-qwen25vl] model_path        =", args.model_path)
    print("[hallusion-cache-qwen25vl] bench_file        =", bench_file)
    print("[hallusion-cache-qwen25vl] image_folder      =", image_folder)
    print("[hallusion-cache-qwen25vl] cache_folder      =", cache_folder)
    print("[hallusion-cache-qwen25vl] cache_dtype       =", args.cache_dtype)
    print("[hallusion-cache-qwen25vl] overwrite         =", args.overwrite)
    print("[hallusion-cache-qwen25vl] limit             =", args.limit)
    print("[hallusion-cache-qwen25vl] only_last_n       =", args.only_last_n)
    print("[hallusion-cache-qwen25vl] only_images_file  =", args.only_images_file)
    print("[hallusion-cache-qwen25vl] min_pixels        =", args.min_pixels)
    print("[hallusion-cache-qwen25vl] max_pixels        =", args.max_pixels)
    print("[hallusion-cache-qwen25vl] max_visual_tokens =", args.max_visual_tokens)
    print("[hallusion-cache-qwen25vl] legacy_save       =", args.legacy_save)
    print("[hallusion-cache-qwen25vl] trust_remote_code =", bool(args.trust_remote_code))
    print("[hallusion-cache-qwen25vl] use_fast          =", use_fast)
    print("=" * 100)
    ...
    items = load_hallusionbench(bench_file)
    all_fns = collect_unique_filenames_keep_order(items)
    sel_fns = select_filenames(all_fns, args)

    print(f"[hallusion-cache-qwen25vl] unique visual images (all) = {len(all_fns)}")
    print(f"[hallusion-cache-qwen25vl] selected images            = {len(sel_fns)}")
    if len(sel_fns) <= 60:
        print(f"[hallusion-cache-qwen25vl] selected preview          = {sel_fns}")

    print("[hallusion-cache-qwen25vl] loading pure visual ImageProcessor ...")
    ip, ip_src = load_visual_image_processor(
        args.model_path,
        trust_remote_code=bool(args.trust_remote_code),
        use_fast=use_fast,
    )
    print("[hallusion-cache-qwen25vl] visual processor source =", ip_src)
    print("[hallusion-cache-qwen25vl] visual processor class  =", type(ip))
    if hasattr(ip, "size"):
        print("[hallusion-cache-qwen25vl] ip.size =", getattr(ip, "size", None))
    if hasattr(ip, "min_pixels"):
        print("[hallusion-cache-qwen25vl] ip.min_pixels =", getattr(ip, "min_pixels", None))
    if hasattr(ip, "max_pixels"):
        print("[hallusion-cache-qwen25vl] ip.max_pixels =", getattr(ip, "max_pixels", None))

    num_saved = num_skipped = num_failed = num_too_big = 0

    with torch.inference_mode():
        for rel_fn in tqdm(sel_fns, desc="Caching HallusionBench images (Qwen2.5-VL, pure vision)"):
            ...
            try:
                ...
                if args.max_visual_tokens and args.max_visual_tokens > 0:
                    if est_tokens > int(args.max_visual_tokens):
                        ...
                        if args.verbose_per_image:
                            print(f"[warn][too_big] {rel_fn}: est_tokens={est_tokens} > {args.max_visual_tokens}")
                        continue
                ...
                meta: Dict[str, Any] = {
                    ...
                    "pixel_values_format": (
                        "patch_seq" if pv.dim() == 2 else ("chw" if (pv.dim() == 3 and pv.shape[0] == 3) else "other")
                    ),
                ...
                if args.verbose_per_image:
                    ...
                    print(f"[ok] {rel_fn}: pv_shape={tuple(pv.shape)}, grid={meta['image_grid_thw']}, "
                          f"tokens={est_tokens}, file={size_mb:.2f}MB")
    ...
    print("=" * 100)
    print(f"[hallusion-cache-qwen25vl] saved   = {num_saved}")
    print(f"[hallusion-cache-qwen25vl] skipped = {num_skipped}")
    print(f"[hallusion-cache-qwen25vl] too_big = {num_too_big}")
    print(f"[hallusion-cache-qwen25vl] failed  = {num_failed}")
