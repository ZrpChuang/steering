# 1.朴素的steering算法
# 2.klgate的算法
# 3.多种向量，多种注入层，多种注入lamda
# 4.明确的生成目录，明确的后处理方式

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
COCO(CHAIR) caption generation + sweep for steering (fixed vs KL-gated)
======================================================================

Features
--------
1) Switch steering mode:
   --steer-mode {none,fixed,klgate}
   - none  : baseline (no steering)
   - fixed : constant lambda (lambda_t ≡ lambda_const)
   - klgate: token-level KL-gated lambda_t

2) Multiple probes / vectors:
   --probe-paths "p1.npz,p2.npz,p3.npz"

3) Multiple layer schemes + lambda grid:
   --layer-schemes "1,2,3;15,16,17,18;25,26,27,28,29,30"
   --lambda-fixed-grid "0.5,1.0,2.0"     (for fixed)
   --lambda-max-grid   "2.4,2.7,3.0"     (for klgate)

4) Clear output directory and file names:
   exp_folder/
     mode=klgate/
       probe=xxx/
         layers=1-30/
           lammax=2p7_lammin=0/
             coco_subset500_seed1994_t0_topP1_topK0_mnt512_vs-coupled_tau1_gateb2p25_capmargin_lamcap2.jsonl
             meta.json

Notes
-----
- KL-gated mode costs more: each token requires >= 2 forwards (img+noimg),
  cache can only reduce image preprocess, cannot reduce extra forwards.
"""
...
def list_coco_images(data_path: str) -> List[str]:
    """
    returns list of image filenames (not full paths), sorted
    """
    files = []
...
def build_run_dir(
    ...
    """
    Hierarchical directory style (your preferred style #1).
    """
    p = os.path.join(
        exp_folder,
...
def build_jsonl_filename(args, subset_size: int, steer_mode: str) -> str:
    ...
    if steer_mode == "fixed":
        # fixed mode key hyperparams included by directory lam=...
        name = f"coco_{subset}_{seed}_{ttag}_{ptag}_{ktag}_{mnt}_fixed.jsonl"
        return name

    if steer_mode == "klgate":
        # include gating hyperparams
        ...
        name = f"coco_{subset}_{seed}_{ttag}_{ptag}_{ktag}_{mnt}_{vs}_{tau}_{vb}_{gs}_{cap}_{lcap}.jsonl"
        return name

    # none
    name = f"coco_{subset}_{seed}_{ttag}_{ptag}_{ktag}_{mnt}_none.jsonl"
    return name
...
def parse_args():
    p = argparse.ArgumentParser("COCO(CHAIR) caption sweep with fixed / KL-gated steering")

    # ------------- io -------------
    p.add_argument("--exp-folder", type=str, default="/nas_data/ruipeng.zhang/chair_eval/llava_3th",
    ...
    # ------------- fixed prompt (as per your requirement) -------------
    # prompt is fixed in code, no CLI switch.
    ...
    # fixed lambda sweep
    p.add_argument("--lambda-fixed-grid", type=str, default="0.7,0.9,1.1,1.3,1.5,1.7",
    ...
    # klgate lambda sweep
    p.add_argument("--lambda-max-grid", type=str, default="0.9,1.2,1.5,2.1,2.4,2.7,3.0,3.5,4.0",
    ...
    # KL gating params (defaults aligned to your previous script)
    ...
    # cap (aligned to your previous script)
    ...
    p.add_argument("--image-cache-folder", type=str, default="/nas_data/ruipeng.zhang/COCO_val2014_pre_cache_llava",
                   help="optional folder with cached pixel_values: <image_file>.pt")

    # debug
    p.add_argument("--log-every", type=int, default=0)
    p.add_argument("--debug", action="store_true")
    p.add_argument("--debug-first-n", type=int, default=0,
                   help="if >0, only first N images print debug logs (avoid spam)")
...
def make_grid_from_args(grid_str: str, run_str: str) -> List[float]:
    ...
    # keep order, dedup, intersect with grid
    seen = set()
    run_u = []
    for x in run:
        if x not in seen:
...
def main():
    ...
    # fixed prompt (as you required)
    FIXED_PROMPT = "Please help me describe the image in detail."
    ...
    # init runtime once (avoid re-loading model per run)
    rt = LlavaSteeringRuntime(
    ...
    # snapshot baseline steering state (for reset between runs)
    ...
    # build run configs list (each is one output jsonl)
    run_confs = []
    ...
    for run_idx, conf in enumerate(run_confs):
        ...
        if steer_mode in ("fixed", "klgate"):
            rt.inject_fixed_from_probe(
                ...
                steer_layers=layers,
                lambda_scale=0.0,  # important: actual lambda controlled by silent_set_lambda_fixed
    ...
    if args.save_summary:
        summary_path = os.path.join(args.exp_folder, "coco_chair_sweep_summary.json")
