...
def _plot_one_axis(ax, x: np.ndarray, delta: np.ndarray, colors: List[str],
    ...
    # hatch can look strong if alpha too high; keep very light
    ax.axhspan(-tau, +tau, facecolor="grey", alpha=0.06, hatch="..", edgecolor="grey", linewidth=0.0, zorder=0.1)
...
def summarize_token_weighted(mets: List[Tuple[int, Dict[str, float]]]) -> Dict[str, float]:
    ...
    for L, m in mets:
        ...
        if w <= 0:
            continue
        wsum += w
        # if nan appears, the final may become nan; keep simple/transparent
...
def main():
    ...
    for i, c in enumerate(cases_sorted, 1):
        ...
        try:
            ...
            line = (
                f"{i:04d}\t{sid}\t{score:.6f}\tOK\t{os.path.basename(out_png)}\t"
            ...
            report_lines.append(
                f"{i:04d}\t{sid}\t{score:.6f}\tFAILED\t{os.path.basename(out_png)}\t"
