...
- Drop a few "Global bad_hallu (x<0,y<=0)" cases to make N a round number (default 180).
...
OUT_DIR = os.path.join(RUN_DIR, "aggregate_plots")

# target N (183 -> 180 by default)
TARGET_N = 180

# If TARGET_N is None: keep all
# If TARGET_N < N: drop from global bad_hallu first, then global bad_both if still needed.
DROP_FROM_BAD_HALLU_FIRST = True

# axis limit strategy
# "percentile" is usually best to reduce outlier influence.
...
# 2) Quadrant logic
# -----------------------------
# Quadrant naming (match your plot text)
...
QUAD_PRETTY = {
    "ideal": "ideal (x>=0,y<=0)",
    "sledgehammer": "sledgehammer (x>=0,y>0)",
    "bad_hallu": "bad_hallu (x<0,y<=0)",
    "bad_both": "bad_both (x<0,y>0)",
}
...
# -----------------------------
# 4) Subset selection (drop a few bad_hallu to round N)
# -----------------------------
def make_subset(cases: List[Dict[str, Any]], target_n: int) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    meta = {
        ...
        "dropped_ids": [],
        "drop_policy": {
            "priority": ["global.bad_hallu (most negative x first)", "global.bad_both (most negative x first)"],
        },
    }
    ...
    for c in valid:
        ...
        qg = quadrant_name(xg, yg)
        if qg == "bad_hallu":
            bad_hallu.append((xg, int(c["id"]), c))
        elif qg == "bad_both":
            bad_both.append((xg, int(c["id"]), c))
        else:
            keep.append(c)

    # sort by xg ascending (most negative first) to drop the noisiest
    ...
    # remaining bad_hallu not dropped are kept
    for j in range(i, len(bad_hallu)):
        keep.append(bad_hallu[j][2])

    # 2) if still need, drop from bad_both
    k = 0
    ...
    # remaining bad_both not dropped are kept
    for j in range(k, len(bad_both)):
        keep.append(bad_both[j][2])

    # If still need_drop >0 (unlikely), drop from overall most-negative xg
    if need_drop > 0:
        rest = []
        for c in keep:
            xg, _yg = extract_xy(c, "global")
            rest.append((xg, int(c["id"]), c))
        rest.sort(key=lambda t: t[0])  # most negative xg first
        kept2 = []
        for idx, (xg, sid, c) in enumerate(rest):
            if need_drop > 0:
                dropped.append(sid)
                need_drop -= 1
            else:
...
def main():
    ...
    # Save subset meta + ids (for paper reproducibility)
    ...
    # --- (1) Scatter: Global vs Soft, count+percent ---
    ...
    ax.set_title(f"Quadrant composition (subset N={N})")
    ax.set_ylim(0, 100)
    ax.grid(True, axis="y", alpha=0.25)
    ax.legend(loc="upper right", framealpha=0.9)

    # annotate counts in text
    txt = (
    ...
    ax.set_title(f"Quadrant transition (Global → Soft), subset N={N}", fontsize=14)
    ...
    # highlight the migration you care about: sledgehammer -> ideal
    # (global row=1, soft col=0)
